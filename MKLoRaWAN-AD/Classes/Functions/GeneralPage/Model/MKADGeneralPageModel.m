...
@interface MKADGeneralPageModel ()
...
@implementation MKADGeneralPageModel
...
- (BOOL)readInterval {
    ...
    [MKADInterface ad_readHeartbeatIntervalWithSucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configInterval {
    __block BOOL success = NO;
    [MKADInterface ad_configHeartbeatInterval:[self.interval integerValue] sucBlock:^{
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
