...
@interface MKADLoRaController ()<UITableViewDelegate, UITableViewDataSource>

@property (nonatomic, strong)MKBaseTableView *tableView;

@property (nonatomic, strong)NSMutableArray *dataList;

@property (nonatomic, strong)MKADLoRaPageModel *dataModel;

@end

@implementation MKADLoRaController

- (void)dealloc {
    NSLog(@"MKADLoRaController销毁");
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.row == 1) {
        //Connection Settings
        MKADLoRaSettingController *vc = [[MKADLoRaSettingController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.row == 2) {
        //Application Settings
        MKADLoRaAppSettingController *vc = [[MKADLoRaAppSettingController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
}
...
- (MKADLoRaPageModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKADLoRaPageModel alloc] init];
    }
    return _dataModel;
}
