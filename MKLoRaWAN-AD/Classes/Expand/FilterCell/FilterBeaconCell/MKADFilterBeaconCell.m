...
@implementation MKADFilterBeaconCellModel
@end

@interface MKADFilterBeaconCell ()
...
@implementation MKADFilterBeaconCell

+ (MKADFilterBeaconCell *)initCellWithTableView:(UITableView *)tableView {
    MKADFilterBeaconCell *cell = [tableView dequeueReusableCellWithIdentifier:@"MKADFilterBeaconCellIdenty"];
    if (!cell) {
        cell = [[MKADFilterBeaconCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"MKADFilterBeaconCellIdenty"];
    }
    return cell;
}
...
#pragma mark - setter
- (void)setDataModel:(MKADFilterBeaconCellModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKADFilterBeaconCellModel.class]) {
        return;
    }
...
- (MKTextField *)minTextField {
    if (!_minTextField) {
        ...
        _minTextField.textChangedBlock = ^(NSString * _Nonnull text) {
            @strongify(self);
            if ([self.delegate respondsToSelector:@selector(mk_ad_beaconMinValueChanged:index:)]) {
                [self.delegate mk_ad_beaconMinValueChanged:text index:self.dataModel.index];
            }
        };
    }
...
- (MKTextField *)maxTextField {
    if (!_maxTextField) {
        ...
        _maxTextField.textChangedBlock = ^(NSString * _Nonnull text) {
            @strongify(self);
            if ([self.delegate respondsToSelector:@selector(mk_ad_beaconMaxValueChanged:index:)]) {
                [self.delegate mk_ad_beaconMaxValueChanged:text index:self.dataModel.index];
            }
        };
    }
